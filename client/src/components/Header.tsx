...
import { Dialog, Button, DialogActions, DialogContent, DialogTitle, TextField, Icon, AppBar, Toolbar, IconButton } from '@material-ui/core'
...
class Header extends Component<Props, State> {
    ...
    renderToolbarRightButton() {
        ...
        if (this.props.isHome) {
            // we show a '+' button
            return (<IconButton color="inherit" onClick={() => this.props.history.push('/details')}><Icon>add</Icon></IconButton>)
        } else {
            // invisible button (trick)
            return (<IconButton color="primary"><Icon>add</Icon></IconButton>)
        }
    }
    ...
    renderToolbarLeftButton() {
        if (this.props.isHome) {
            return (<IconButton color="inherit" onClick={() => this.openDialog()}><Icon>search</Icon></IconButton>)
        } else {
            return (<IconButton color="inherit" onClick={() => this.goBack()}><Icon>arrow_back</Icon></IconButton>)
        }
    }
        ...
        return (
            <AppBar color="primary" position="sticky">
                <Toolbar style={{flex:1, flexDirection:'row', justifyContent:'space-between'}}>
                    ...
                    <IconButton color="inherit" onClick={() => this.props.history.push('/')}>SPA</IconButton>
                ...
                <Dialog
                    ...
                    <DialogContent>
                        <form onSubmit={(event) => {event.preventDefault(); this.handleSearchDialogSubmit();}}>
                            <TextField
                            autoFocus
                            id="rfid"
                            ...
                            type="text"
                            fullWidth
                            onChange={(event) => this.handleSearchDialogTextfieldChange(event)}
                            helperText={(this.state.errorMessage.length) ? this.state.errorMessage : ''}
                    ...
                    <DialogActions>
                        ...
                        <Button onClick={() => this.handleSearchDialogSubmit()} color="primary">
                        Find
                        </Button>
                    </DialogActions>
                </Dialog>
